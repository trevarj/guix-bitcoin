...
are supplied so npm can reach the registry."
  (computed-file
   (string-append name "-node-modules")
   (with-imported-modules (source-module-closure '((guix build utils)))
     #~(begin
         ...
         (copy-recursively (string-append scratch "/node_modules")
                           #$output)
         ;; Drop npm's own bookkeeping copy of the lockfile
         ;; (its mtime/ordering is install-run dependent).
         (let ((stray (string-append #$output "/.package-lock.json")))
